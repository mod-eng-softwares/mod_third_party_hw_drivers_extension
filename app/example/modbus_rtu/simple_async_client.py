...
async def run_async_simple_client(comm, host, port, framer=FramerType.SOCKET):
    ...
    try:
        ...
        rr = await client.read_coils(1000, count=1, device_id=10)
    ...
    try:
        # See all calls in client_calls.py
        rr = await client.read_input_registers(2000, count=2, device_id=10)
    except ModbusException as exc:  # pragma: no cover
        print(f"Received ModbusException({exc}) from library")
        client.close()
        return
...
if __name__ == "__main__":
    asyncio.run(
        run_async_simple_client("serial", host="", port="COM33", framer=FramerType.RTU), debug=True
    )
